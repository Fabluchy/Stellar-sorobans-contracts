...
impl MigrationFramework for DefaultMigrationFramework {
    ...
    fn begin_migration(
        ...
        self.validate_migration(env, steps.clone())?;
    ...
    fn validate_migration(&self, _env: &Env, steps: Vec<MigrationStep>) -> Result<(), MigrationError> {
        ...
        if steps.len() > 1 {
            for i in 0..(steps.len() - 1) {
                let current = steps.get(i).unwrap();
                let next = steps.get(i + 1).unwrap();
                if current.to_version != next.from_version {
                    return Err(MigrationError::ValidationFailed);
                }
            }
        }
...
impl DefaultMigrationFramework {
    ...
    fn generate_migration_id(&self, env: &Env) -> u64 {
        ...
        let hash = env.crypto().sha256(&contract_address.to_xdr(env));
        
        // Combine timestamp and hash for uniqueness
        (timestamp ^ (hash.get(0).unwrap_or(0) as u64) | ((hash.get(1).unwrap_or(0) as u64) << 8))
    }
